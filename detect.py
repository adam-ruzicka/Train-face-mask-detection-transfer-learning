...
maskClassifier = load_model('mask_detector.model')  # Mask classifier
...
while True:
    ...
    for (x, y, h, w) in faces:
        ...
        faceROI = cv2.resize(faceROI, (224, 224))  # Resizing faceROI to 224x224
        # Because, Our model accepts 224x224 as input
        faceROI = img_to_array(faceROI)
        faceROI = faceROI.reshape(1, 224, 224, 3)  # Changing dimensions to 1x224x224x3, Because our model
        # takes input as 4D matrix(BATCH_SIZE, 224, 224, #Channels)
        ...
        (label, color, prob) = ('S maskou', (0, 255, 0), withmask * 100.0) if withmask > withoutmask else (
            'Bez masky', (0, 0, 255), withoutmask * 100.0)
